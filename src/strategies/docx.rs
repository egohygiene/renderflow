...
impl OutputStrategy for DocxStrategy {
    fn render(&self, ctx: &RenderContext) -> Result<()> {
        if ctx.dry_run {
            info!(input = %ctx.input_path, output = %ctx.output_path, "[dry-run] Would render DOCX output via pandoc");
            return Ok(());
        }
...
mod tests {
    ...
    #[test]
    fn test_docx_strategy_dry_run_skips_execution() {
        let vars = HashMap::new();
        let strategy = DocxStrategy::new(None, "templates".to_string());
    ...
    #[test]
    fn test_docx_strategy_dry_run_with_missing_template_skips_execution() {
        let vars = HashMap::new();
        let strategy = DocxStrategy::new(
            Some("nonexistent.docx".to_string()),
